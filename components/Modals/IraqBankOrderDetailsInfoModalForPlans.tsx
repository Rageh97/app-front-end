...
interface IraqBankOrderDetailsInfoModalPlansProps {
  modalOpen: boolean;
  packDetails: any;
  setModalOpen: Function;
}

const IraqBankOrderDetailsInfoModalForPlans: React.FC<
IraqBankOrderDetailsInfoModalPlansProps
> = ({ modalOpen, setModalOpen, packDetails }) => {
  // Debug log when component renders or packDetails changes
  useEffect(() => {
    console.log('CIH Bank Modal - packDetails:', {
    ...
    >
      <Dialog.Panel className="w-full max-w-[500px] text-center">
        <div className="flex flex-col overflow-hidden w-full rounded-2xl bg-white shadow-2xl relative">
          ...
          {/* Invoice Content */}
          <div className="px-8 py-6 space-y-6 bg-[#190237]">
              ...
              <div className="bg-gradient-to-br from-[#190237] to-[#2a0f4f] rounded-xl p-5 text-white shadow-lg">
                <div className="space-y-4">
                  <div className="flex justify-between items-center pb-3 border-b border-white/10">
                    ...
                    <span className="font-bold text-lg">IRAQ BANK</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-orange font-semibold text-sm">{t('bankDetails.accountNumber')}</span>
                    <span className="font-bold text-lg tracking-wider">07824931630</span>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <span className="text-white font-bold text-right max-w-[60%]">
                      {packDetails?.isDevice 
                        ...
                        : packDetails?.period === 'year' 
                          ? `${t('bankDetails.yearOf')} ${packDetails?.pack_name} ${t('bankDetails.pack')}`
                          : packDetails?.period === 'day'
                            ? `${t('bankDetails.dayOf')} ${packDetails?.pack_name} ${t('bankDetails.pack')}`
                            : `${t('bankDetails.monthOf')} ${packDetails?.pack_name} ${t('bankDetails.pack')}`}
                    </span>
                  </div>
...
export default IraqBankOrderDetailsInfoModalForPlans;
