...
import { ArrowRight, Users, Download, X, RefreshCw, Wand2, CreditCard, Crown, ChevronLeft, ArrowLeft, ShieldCheck, Sparkles, Play, Film, Layers, Heart, MessageCircle, Share2, Trash2 } from 'lucide-react';
...
const UGC_STYLES = [
    { id: 'authentic', name: 'أصيل', icon: <Heart size={16} />, desc: 'محتوى طبيعي وحقيقي' },
    { id: 'trendy', name: 'عصري', icon: <Share2 size={16} />, desc: 'يواكب الترندات الحالية' },
    { id: 'casual', name: 'عادي', icon: <MessageCircle size={16} />, desc: 'بسيط ومريح يومي' },
];
...
export default function UGCPage() {
      ...
      if (res.status === 200) {
        ...
        if (data.success) {
            setUserMedia(data.media.map((v: any) => ({
                id: v.media_id,
                url: v.media_url || v.cloudinary_url,
                prompt: v.prompt
            })));
        }
      }
    } catch (e) {} finally { setLoadingMedia(false); }
  };
  ...
  const deleteMedia = async (mediaId: number) => {
      ...
      try {
          await fetch(`${apiBase}/api/ai/user-media/${mediaId}`, {
              method: 'DELETE',
  ...
  const deleteAllMedia = async () => {
      ...
      setUserMedia([]);
      try {
          await fetch(`${apiBase}/api/ai/user-media?type=video`, {
              method: 'DELETE',
              headers: { 'Authorization': token as any, "User-Client": (global as any)?.clientId1328 }
          });
              ...
              <div className="bg-[#080808] rounded-[3rem] border border-white/5 min-h-[500px] lg:min-h-[850px] flex items-center justify-center relative overflow-hidden group shadow-inner">
                {result ? (
                  <div className="relative w-full h-full p-8 flex flex-col items-center justify-center group/vid">
                    ...
                    <video src={result.video_url} controls className="max-h-[650px] w-full max-w-sm rounded-[2.5rem] shadow-2xl border border-white/10 animate-fade-in aspect-[9/16]" />
