            ...
            <aside className="w-[320px] md:w-[360px] border-l border-white/10 bg-[#050505] overflow-y-auto custom-scrollbar flex flex-col p-5 shrink-0">
                <div className="space-y-6">
                    <div className="space-y-3">
                        <label className="text-xs font-bold text-gray-500 flex items-center gap-2 uppercase tracking-wide"><Upload size={14} className="text-rose-400" /> صورة المنتج</label>
                        <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-white/5 rounded-2xl p-6 text-center hover:bg-rose-500/5 hover:border-rose-500/40 cursor-pointer transition-all">
                            {uploadedProduct ? (
                                <div className="space-y-2">
                                    <img src={uploadedProduct} className="h-40 mx-auto rounded-xl object-cover shadow-2xl" />
                                    <span className="text-[10px] text-rose-400 font-bold block">تغيير المنتج</span>
                                </div>
                            ) : (
                                <div className="py-8">
                                    <Plus className="mx-auto text-gray-600 mb-2" />
                                    <p className="text-[10px] text-gray-400 font-medium">ارفع صورة المنتج بخلفية واضحة</p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="space-y-3">
                         <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">بيئة التصوير</label>
                         <div className="grid grid-cols-2 gap-2">
                             {BACKGROUND_OPTIONS.map(bg => (
                                 <button key={bg.id} onClick={() => setSelectedBackground(bg.id)} className={`flex items-center gap-2 p-3 rounded-xl border text-right transition-all ${selectedBackground === bg.id ? 'bg-rose-500/10 border-rose-500/40 text-rose-300' : 'bg-white/5 border-white/5 text-gray-500 hover:bg-white/10'}`}>
                                     {bg.icon} <span className="text-[11px] font-bold">{bg.name}</span>
                                 </button>
                             ))}
                         </div>
                    </div>
                    ...
                    <div className="space-y-3">
                         <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">تعليمات إضافية (اختياري)</label>
                         <textarea value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} placeholder="مثال: إضاءة دافئة، خلفية رخامية..." className="w-full h-24 p-3 rounded-xl bg-white/5 border border-white/10 text-xs focus:border-rose-500/40 outline-none resize-none transition-all" />
                    </div>
                </div>
                ...
                <div className="mt-auto pt-6 border-t border-white/5">
                    ...
                    <PremiumButton label={isGenerating ? "جاري الإخراج..." : "بدء الجلسة"} icon={isGenerating ? RefreshCw : Camera} onClick={onGenerate} disabled={!uploadedProduct || isGenerating} className="w-full py-4 text-sm rounded-xl" />
                </div>
            </aside>
                    ...
                    {plans.map((p: any) => (
                      <button key={p.plan_id} onClick={() => { setSelectedPlan(p); setShowBuyModal(false); setOpenPaymentModal(true); }} className="w-full p-4 bg-white/5 hover:bg-white/10 rounded-2xl text-right transition-all duration-300 border border-white/5 hover:border-blue-500/50 group">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-white font-bold text-xl bg-white/10 px-3 py-1 rounded-lg group-hover:bg-blue-500 group-hover:text-white transition-all">${p.amount}</div>
                        </div>
                      </button>
                    ))}
                  </div>
