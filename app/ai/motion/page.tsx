...
import { ArrowRight, Zap, Upload, Download, X, RefreshCw, Wand2, CreditCard, Crown, ChevronLeft, ArrowLeft, ShieldCheck, Sparkles, Play, Film, Layers, Image as ImageIcon, Trash2 } from 'lucide-react';
...
const MOTION_TYPES = [
    { id: 'subtle', name: 'خفيفة', icon: <Sparkles size={16} />, desc: 'حركة طبيعية وهادئة' },
    { id: 'dynamic', name: 'ديناميكية', icon: <Zap size={16} />, desc: 'حركة واضحة ونشطة' },
    { id: 'cinematic', name: 'احترافية', icon: <Film size={16} />, desc: 'مظهر سينمائي درامي' },
];
...
export default function MotionSimulatorPage() {
  ...
  const onProcess = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/ai/motion-effects`, {
        method: "POST",
        headers: { 'Authorization': token as any, 'Content-Type': 'application/json', "User-Client": (global as any)?.clientId1328 },
        body: JSON.stringify({ image: imageFile, motion_type: motionType }),
      });
      ...
      if (data.success) {
        ...
        toast.success('تم تحويل الصورة لمقطع فيديو!');
      } else {
  ...
  const fetchUserVideos = async () => {
    ...
    try {
        const res = await fetch(`${apiBase}/api/ai/user-videos?limit=8`, {
            headers: { 'Authorization': token as any, 'Content-Type': 'application/json', "User-Client": (global as any)?.clientId1328 }
        });
        if (res.status === 200) {
            const data = await res.json();
  ...
  const deleteAllVideos = async () => {
      if (!confirm('حذف جميع الفيديوهات؟')) return;
      if (!apiBase) return;
      const token = getToken();
      setUserVideos([]);
      try {
  ...
  useEffect(() => {
     if (balance) { // Ensure fetchBalance happened or similar trigger
         fetchUserVideos();
     }
  }, [balance]);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#000000] text-white selection:bg-cyan-500/30 font-sans" dir="rtl">
        {/* Background Ambient */}
        <div className="fixed inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-10 pointer-events-none"></div>
        <div className="fixed top-[-10%] right-[-10%] w-[50%] h-[50%] bg-cyan-900/5 blur-[120px] rounded-full pointer-events-none"></div>
        <div className="fixed bottom-[-10%] left-[-10%] w-[50%] h-[50%] bg-blue-900/5 blur-[120px] rounded-full pointer-events-none"></div>
          ...
          <div className="max-w-[1600px] mx-auto px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <span className="text-xl font-bold">محاكي الحركة Motion Simulator</span>
              </div>
               ...
               <div className="flex items-center gap-4">
                 <div className="hidden md:flex items-center gap-3 px-4 py-2 bg-white/5 rounded-xl border border-white/10">
                  {loadingBalance ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm font-bold ${balance.remaining_credits === 0 ? 'text-red-400' : 'text-cyan-400'}`}>
                ...
                >
                  <span className="absolute inset-[-1000%] animate-[spin_3s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#06b6d4_0%,#3b82f6_50%,#06b6d4_100%)]"></span>
                  <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-lg bg-[#050505] px-4 text-xs font-black text-white backdrop-blur-3xl gap-2 transition-all hover:bg-black/40">
                    <Crown size={14} className="text-cyan-500" />
                    شراء رصيد
                  </span>
                </button>
              </div>
            ...
            <div className="order-1 lg:col-span-4 space-y-4 lg:sticky lg:top-28">
               <div className="mb-2">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-400">Life Simulator</span>
                </div>
                <div className="text-sm text-gray-500 font-bold leading-relaxed">
                   حول صورك الصامتة إلى لحظات تنبض بالحياة عبر إضافة حركة سينمائية ذكية.
                </div>
              </div>

               <div className="bg-[#0c0c0c] rounded-[2rem] p-6 border border-white/5 relative group shadow-2xl overflow-hidden mb-4">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 opacity-50"></div>
                
                <div className="relative space-y-6">
                  <div>
                    <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest block mb-3">الصورة المراد تحريكها</span>
                    <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" id="motion-up" />
                    <label htmlFor="motion-up" className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5 cursor-pointer hover:border-cyan-500/30 transition-all">
                        <span className="text-xs text-gray-400 max-w-[200px] truncate">{imageFile ? "تم اختيار صورة" : "ارفع صورتك هنا..."}</span>
                        <ImageIcon size={18} className="text-cyan-500" />
                    </label>
                  </div>

                  <div>
                     <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest block mb-4">نمط الحركة</span>
                         ...
                         {MOTION_TYPES.map((m) => (
                             <button
                                ...
                                className={`flex items-center gap-4 p-4 rounded-xl transition-all duration-300 border ${
                                    motionType === m.id
                                    ? 'bg-cyan-500/10 border-cyan-500/50 text-cyan-300 shadow-[0_0_20px_rgba(6,182,212,0.1)]'
                                    : 'bg-white/5 border-white/5 text-gray-500 hover:border-white/10'
                                }`}
                             >
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${motionType === m.id ? 'bg-cyan-500/20' : 'bg-white/5'}`}>
                                    {m.icon}
                                </div>
                  ...
                  <div className="mt-8 border-t border-white/5 pt-6">
                    <PremiumButton 
                        label={isProcessing ? "جاري محاكاة الحركة..." : " بدء الانشاء"}
              ...
              <div className="bg-[#080808] rounded-[3rem] border border-white/5 min-h-[500px] lg:min-h-[850px] flex items-center justify-center relative overflow-hidden group shadow-inner">
                {result ? (
                  <div className="relative w-full h-full p-8 flex flex-col items-center justify-center group/vid">
                    ...
                    <video src={result.video_url} controls autoPlay loop className="max-h-[650px] w-full max-w-2xl rounded-[2.5rem] shadow-2xl border border-white/10 animate-fade-in" />
                    <div className="mt-8 flex items-center gap-3">
                        ...
                        <button onClick={() => setResult(null)} className="p-4 bg-white/5 hover:bg-white/10 rounded-2xl transition-all border border-white/10" title="إعادة التوليد">
                            <RefreshCw size={20} />
                        </button>
                    </div>
                  </div>
                  ...
                  <div className="text-center relative z-10 w-full max-w-sm px-8">
                    <div className="w-24 h-24 relative mx-auto mb-8">
                        <div className="absolute inset-0 rounded-[2.5rem] border-4 border-cyan-500/10 scale-125"></div>
                        <div className="absolute inset-0 rounded-[2.5rem] border-4 border-t-cyan-500 animate-spin"></div>
                        <Sparkles className="absolute inset-0 m-auto text-cyan-400 animate-pulse" size={40} />
                    </div>
                    <h3 className="text-2xl font-black mb-2">جاري محاكاة الإطارات...</h3>
                    <div className="w-full bg-white/5 rounded-full h-1.5 overflow-hidden mt-6">
                      <div className="bg-cyan-500 h-full transition-all duration-700" style={{ width: `${processingProgress}%` }}></div>
                    </div>
                  </div>
                ) : (
                  <div className="text-center relative z-10 p-12">
                     <div className="w-32 h-32 bg-white/[0.02] rounded-[3rem] flex items-center justify-center mx-auto mb-8 border border-white/5 group-hover:scale-105 transition-all duration-700 shadow-inner">
                      <Zap size={64} className="text-white/5 group-hover:text-cyan-500/10 transition-colors" />
                    </div>
                    <p className="text-gray-600 max-w-xs mx-auto font-bold text-lg leading-relaxed">ارفع صورتك المفضلة، واختر نمط الحركة، وشاهد كيف تصبح حية.</p>
                  </div>
                )}
              </div>

               {/* Previous Works Section - Motion */}
               <div className="mt-12 lg:col-span-12 border-t border-white/5 pt-8">
                     <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                           ...
                           <div>
                              <h3 className="text-lg font-bold text-white">تحريكاتك السابقة</h3>
                              <p className="text-xs text-gray-500 font-medium">سجل الصور التي قمت ببث الحياة فيها</p>
                           </div>
                        </div>
                     ...
                     {loadingVideos ? (
                        ...
                        <div className="py-12 text-center border border-dashed border-white/10 rounded-2xl bg-white/[0.02]">
                           ...
                           <p className="text-gray-400 font-bold text-sm">لا توجد سجلات سابقة</p>
                        </div>
                     )}
                  </div>
            </div>
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto">
              {loadingPlans ? <div className="text-center py-12 animate-pulse text-gray-500">جاري التحميل...</div> : (
                <div className="space-y-4">
                  {plans.map((p) => (
                    <button key={p.plan_id} onClick={() => { setSelectedPlan(p as any); setShowBuyModal(false); setOpenPaymentModal(true); }} className="w-full p-4 bg-white/5 hover:bg-white/10 rounded-2xl text-right transition-all border border-white/5 hover:border-cyan-500/50 group flex items-center justify-between">
                      <div>
                        <div className="font-bold text-white group-hover:text-cyan-400 transition-colors">{p.plan_name}</div>
                        <div className="text-gray-400 text-xs mt-1">{p.credits_per_period} نقطة / {p.period}</div>
                      </div>
                      <div className="text-white font-bold text-xl bg-white/10 px-3 py-1 rounded-lg group-hover:bg-cyan-500">${p.amount}</div>
                    </button>
                  ))}
                </div>
              )}
