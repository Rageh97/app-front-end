...
export default function SketchToImagePage() {
  ...
  const baseCredits = operationPrice('sketch-to-image', 12);
  ...
  const handleDeleteAll = async () => {
    if (!apiBase) return;
    try {
      const res = await fetch(`${apiBase}/api/ai/user-images?tool=sketch`, {
        method: 'DELETE',
        headers: { 'Authorization': getToken() as any, "User-Client": (global as any)?.clientId1328 }
      });
      setUserImages([]);
  ...
  return (
    <>
      ...
      <div className="h-screen flex flex-col bg-[#06070B] text-white selection:bg-emerald-500/30 overflow-hidden" dir="rtl">
        ...
        <AIToolHeader
          title="من رسمة إلى صورة"
          description="تحويل الرسومات التخطيطية والسكتشات إلى صور واقعية مذهلة"
          badge="Sketch to Image"
          icon={Brush}
            ...
            <aside className="w-full lg:w-[380px] h-[calc(100vh-3.5rem)] bg-[#0B0D14] border-b lg:border-b-0 lg:border-l border-white/[0.08] p-5 flex flex-col justify-between shrink-0 overflow-hidden z-30 shadow-2xl relative">
                <div className="flex-1 overflow-y-auto no-scrollbar space-y-4 pr-0.5 pb-2">
                    <div className="flex items-center justify-between">
                        <h2 className="text-lg font-black text-white">من سكتش إلى صورة</h2>
                    </div>

                    <div className="space-y-2">
                        <label className="text-xs font-bold text-gray-400 flex items-center gap-2">
                            <Upload size={14} className="text-emerald-400" /> ارفع رسمتك (Sketch)
                        </label>
                        <div onClick={() => fileInputRef.current?.click()} className="border border-dashed border-white/[0.08] rounded-xl p-4 text-center hover:bg-[#161a27] hover:border-emerald-500/40 cursor-pointer transition-all bg-[#121520]">
                            {sketchImage ? (
                                <div className="space-y-1 relative">
                                    <img src={sketchImage} className="h-28 mx-auto rounded-lg object-contain shadow-2xl" />
                                    <button 
                                    ...
                                    <span className="text-[10px] text-emerald-400 font-bold block">تغيير الرسمة</span>
                                </div>
                            ) : (
                                <div className="py-4">
                                    <Plus size={22} className="mx-auto text-gray-400 mb-1" />
                                    <p className="text-xs text-gray-300 font-bold">انقر لرفع رسمة تخطيطية</p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="space-y-1.5">
                         <label className="text-xs font-bold text-gray-400 flex items-center gap-2">
                            <Type size={13} className="text-emerald-400" /> وصف إضافي للمشهد
                         </label>
                         <div className="rounded-xl border border-white/[0.08] bg-[#121520] p-2.5">
                             <textarea 
                                ...
                                placeholder="مثال: قلعة سحرية عائمة في السماء وسط غيوم وردية بإضاءة سينمائية..." 
                                rows={3}
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-gray-400 flex items-center gap-2">
                            <Wand2 size={13} className="text-emerald-400" /> نمط الصورة
                        </label>
        ...
        <AIResultModal
          ...
          mediaType="image"
          title="من سكتش إلى رسمة وصورة فنية"
          subtitle="تحويل المسودات اليدوية إلى عمل فني متكامل"
          prompt={selectedImage?.prompt}
          details={[
            { label: "الوصف", value: selectedImage?.prompt || "تحويل سكتش" },
                    ...
                    {plans.map((p: any) => (
                      <button key={p.plan_id} onClick={() => { setSelectedPlan(p); setShowBuyModal(false); setOpenPaymentModal(true); }} className="w-full p-4 bg-white/5 hover:bg-white/10 rounded-2xl text-right transition-all duration-300 border border-white/5 hover:border-green-500/50 group">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-white font-bold text-xl bg-white/10 px-3 py-1 rounded-lg group-hover:bg-green-500 group-hover:text-white transition-all">{p.amount} <span className="bg-gradient-to-r from-[#FF0000] via-[#FFFFFF] to-[#000000] bg-clip-text text-transparent font-bold">IQD</span></div>
                        </div>
                      </button>
                    ))}
                  </div>
